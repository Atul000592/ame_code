...
//----------RMB SMS----------//
	...
	public SmsResponse rescheduleAppointmentSMS(SMSTemplateDto smsTemplateDto, String templateId) {
	    logger.info(" >>>>>>>> Reschedule Appointment SMS...Start");
	    Configuration configuration = null;
	    try {
	        configuration = freeMarkerTemplateConfiguration.getInstance();
	    } catch (IOException e) {
	    ...
	    // Set up necessary values for the rescheduling message
	    data.put(SMSConfigurationConstantsAME.IRLA_NUMBER, smsTemplateDto.getIrlaNo());
	    data.put(SMSConfigurationConstantsAME.AME_YEAR, smsTemplateDto.getYear());
	    data.put(SMSConfigurationConstantsAME.FROM_DATE, smsTemplateDto.getFromDate());
	    data.put(SMSConfigurationConstantsAME.TO_DATE, smsTemplateDto.getToDate());
	    
	    // Add the board place, splitting it if needed
	    List<String> chunks = StringChunker.divideStringIntoChunks(smsTemplateDto.getBoardUnitPlace(), 30);
	    String boardPlace = null, boardPlace1 = null;
	    for (int i = 0; i < chunks.size(); i++) {
	        if (i == 0)
	    ...
	    Template smsTemplate = null;
	    try {
	        smsTemplate = configuration.getTemplate("RescheduleAppointmentSMS.ftl");
	    } catch (java.io.IOException e) {
	        e.printStackTrace();
	    }
	    ...
	    // Assuming `SMSAmeUtil.sendMessage` is the method to send the SMS
	     result = SMSAmeUtil.sendMessage(smsTemplateDto.getCountryIsdCode(),
	            smsTemplateDto.getMobileNumber(), smsContent, templateId);

	    logger.info(" >>>>>>>> Reschedule Appointment SMS...End");
