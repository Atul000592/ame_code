...
public class EsignServiceimpl {
	...
	public String createHash(String candidateIrlaNuber, String ameId, String userId, MultipartFile userImage,
			HttpSession session, String baseUrl, String filePath, String fileName, String approvalStatus)
			throws Exception {

		String txnCounter = getTxnCounter();

		baseUrl = "response?rs=" + baseUrl + "/final-response";
		...
		session.setAttribute("tempfilePath", pdfFile.getAbsolutePath());
		session.setAttribute("pdfFile", pdfFile);

		session.setAttribute("candidateIrlaNuber", candidateIrlaNuber);
		...
		String transId = sb.append(esignConfigurationUtil.esginMap.get(StringConstants.transaction_id)).append(random)
				.toString();
		// Create timestamp
		...
		Map<String, Object> documentMap = getDocumentHash(pdfFile.getAbsolutePath(), pdfFile.getAbsolutePath(), null,
				null, null, session, userId, transId + txnTimeStamp + "-" + txnCounter, approvalStatus);
		...
		// String url = "https://nic-esignstaging.nic.in/eSign21/";
		...
		st = "<Esign AuthMode=\"1\" ver=\"2.1\" sc=\"Y\"    ts=\"" + timestamp + "\"  txn =\"" + transId + txnTimeStamp
				+ "-" + txnCounter + "\" ekycId=\"\"  aspId=\""
				+ esignConfigurationUtil.esginMap.get(StringConstants.aspId)
		...
		esignRequestData.setSignDocumentType("AME_FINAL_REPORT");
	...
	public String sign(String xml) throws NoSuchProviderException, UnrecoverableEntryException, KeyStoreException {
		...
		try {
			...
			InputStream is = new FileInputStream(
					esignConfigurationUtil.esginMap.get(StringConstants.path_certFilePath));
			char[] certPassword = esignConfigurationUtil.esginMap.get(StringConstants.certFilePwd).toCharArray();
			ks.load(is, certPassword);
			...
			KeyStore.PrivateKeyEntry keyEntry = (KeyStore.PrivateKeyEntry) ks.getEntry(alias,
					new KeyStore.PasswordProtection(
							esignConfigurationUtil.esginMap.get(StringConstants.certFilePwd).toCharArray()));
	...
	private Map<String, Object> getDocumentHash(String inputFilePath, String outputFilePath, String coOrdinates,
			Integer pageNo, String fileNumber, HttpSession session, String username, String transId,
			String approvredStatus) {
		...
		try {
			...
			if ("Not Approved".equals(approvredStatus)) {
			    appearance.setLayer2Text(
			        "Not Approved And Digitally Signed By " + username + "\nDate: " + sdf.format(cal.getTime()));
			}else {
				appearance.setLayer2Text(
						"Approved And Digitally Signed By " + username + "\nDate: " + sdf.format(cal.getTime()));
			}
			...
			if (coOrdinates != null && pageNo != null) {
				...
				// Assuming 'reader' is your PdfReader and 'appearance' is your
				...
				logger.info(">>>>>> pageHeight " + pageHeight + " >>>> pageHeight " + pageWidth);
				// Signature dimensions
				int rCodeMedical = (int) session.getAttribute("rCodeMedical");

				float signatureWidth = 140.0F; // Width of the signature block
				float signatureHeight = 60.0F; // Height of the signature block
				float xPosition = 0;
				float yPosition = 0;

				if (rCodeMedical == 1 || rCodeMedical == 3 || rCodeMedical == 4) {
					xPosition = 165.0F;
					yPosition = 700.0F;
			...
			logger.info("713890173058 " + transId);
			String appearancepdfWithTransId = transId;
			context.setAttribute(appearancepdfWithTransId, appearance);
	...
	private PdfSignatureAppearance getPdfSignatureAppearanceObject(String inputFilePath, String outputFilePath,
			String coOrdinates, Integer pageNo, String fileNumber, String signee) {
		...
		try {
			...
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy.MM.dd HH:mm:ss z");
			appearance.setLayer2Text("Digitally signed by " + signee + "\nDate: " + sdf.format(cal.getTime()));
			appearance.setRenderingMode(RenderingMode.DESCRIPTION);
			appearance.setAcro6Layers(false);
			appearance.setCertificationLevel(0);
			appearance.setImage((Image) null);
			if (coOrdinates != null && pageNo != null) {
				...
				// Signature dimensions
				float signatureWidth = 110.0F; // Width of the signature block
				float signatureHeight = 40.0F; // Height of the signature block

				// Custom position for the signature
				float xPosition = 410.0F; // 100 points from the left edge of the page
				float yPosition = 520.0F; // 150 points from the bottom edge of the page
			...
			// documentMap.put("appearancepdf", appearance);
			// session.setAttribute("appearancepdf", appearance);
			// context.setAttribute("appearancepdf", appearance);
