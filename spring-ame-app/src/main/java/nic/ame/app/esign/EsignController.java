...
public class EsignController {
	...
	@Autowired
	private ExaminationApplicationFlowService examinationApplicationFlowService;
			...
			if (session != null) {
				...
				if (status) {
					...
					if (rCodeMedical == 3 || rCodeMedical == 4) {
						...
						AmeApprovalProcess ameAppProcessSavedData = this.ameApprovalProcessRepository
								.findByAmeId(ameId);

						Long senderRoleCode = 3l;
						Long reciverRole = 2l;
						...
						ameAppProcessSavedData.setSenderRoleCode(senderRole.get());
						Optional<RefRoleMedical> reciverRoleMedical = refRoleMedicalRepo.findById(reciverRole);
						ameAppProcessSavedData.setReceiverRoleCode(reciverRoleMedical.get());
						ameAppProcessSavedData.setIsEsignStatus(true);
						ameAppProcessSavedData.setrCode("3");
						ameAppProcessSavedData.setUpdatedOn(LocalDateTime.now());
						ameAppProcessSavedData.setUpdatedBy(forcePersonnelOptional.get().getForcePersonalId());
						ameAppProcessSavedData.setIsApproved(true);
						...
						if (optional.isPresent()) {
							...
							ameAppProcessSavedData.setAmeApprovalStatus(ameApprovalStatus);
							ameAppProcessSavedData.setSenderRemark(ameApprovalStatus.getValue());
							ameApprovalProcessRepository.save(ameAppProcessSavedData);
						} else {
						...
						examinationApplicationFlowHistory.setIsEsignStatus(true);
						examinationApplicationFlowHistory.setrCode("3");
						examinationApplicationFlowHistory.setReceiverRoleCode(reciverRoleMedical.get());
						examinationApplicationFlowHistory.setSenderRoleCode(senderRole.get());
						examinationApplicationFlowHistory.setRemark(optional.get().getValue());
						examinationApplicationFlowHistory
								.setUpdatedBy(forcePersonnelOptional.get().getForcePersonalId());
						examinationApplicationFlowHistory.setApproved(true);
						examinationApplicationFlowHistory.setUpdatedOn(LocalDateTime.now());
						this.examinationApplicationFlowService.saveHistory(examinationApplicationFlowHistory);
					...
					if (rCodeMedical == 2) {
						...
						Long reciverRole = 15l;
						int statusCode = CommonConstant.ESIGN_BY_BM_PO;
						AmeApprovalProcess ameAppProcessSavedData = this.ameApprovalProcessRepository
								.findByAmeId(ameId);
						ameAppProcessSavedData.setCreatedBy(forcePersonnelOptional.get().getForcePersonalId());
						ameAppProcessSavedData.setrCode("2");
						ameAppProcessSavedData.setUpdatedOn(LocalDateTime.now());
						Optional<RefRoleMedical> senderRole = refRoleMedicalRepo.findById(senderRoleCode);
						ameAppProcessSavedData.setSenderRoleCode(senderRole.get());
						Optional<RefRoleMedical> reciverRoleMedical = refRoleMedicalRepo.findById(reciverRole);
						...
						Optional<AmeApprovalStatus> optional = ameApprovalStatusRepository.findById(statusCode);
						if (optional.isPresent()) {
							ameApprovalStatus = optional.get();
							ameAppProcessSavedData.setAmeApprovalStatus(ameApprovalStatus);
							ameAppProcessSavedData.setSenderRemark(ameApprovalStatus.getValue());
							ameApprovalProcessRepository.save(ameAppProcessSavedData);
						} else {
						...
						examinationApplicationFlowHistory.setIsEsignStatus(true);
						examinationApplicationFlowHistory.setApproved(true);
						examinationApplicationFlowHistory.setrCode("2");
						examinationApplicationFlowHistory.setReceiverRoleCode(reciverRoleMedical.get());
						examinationApplicationFlowHistory.setSenderRoleCode(senderRole.get());
						examinationApplicationFlowHistory.setRemark(optional.get().getValue());
						examinationApplicationFlowHistory
					...
					if (rCodeMedical == 15) {
						...
						eSignAndFinalAMEReportStatusUpdateService.updateAmeFinalResultAfterEsignAA(ameId,
								medicalCategory, fileName, filePath, originalFilePath);
						Long senderRoleCode = 15l;
						Long reciverRole = 15l;
						int statusCode = CommonConstant.ESIGN_BY_BM_PO_AND_AA;
...
//								forcePersonnelOptional, request, ameId);
						...
						Optional<RefRoleMedical> reciverRoleMedical = refRoleMedicalRepo.findById(reciverRole);
						ameAppProcessSavedData.setReceiverRoleCode(reciverRoleMedical.get());
						ameAppProcessSavedData.setrCode("15");
						ameAppProcessSavedData.setUpdatedOn(LocalDateTime.now());
						ameAppProcessSavedData.setUpdatedBy(forcePersonnelOptional.get().getForcePersonalId());
