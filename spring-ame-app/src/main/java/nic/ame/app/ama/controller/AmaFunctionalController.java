...
//	--------------------------Reschedule appointment-------------------------  //
	...
	public ResponseEntity<?> rescheduleAppointmentForForcePersonnel(@RequestBody Map<String,Object> data,HttpSession httpSession,HttpServletRequest request){
		...
		ForcePersonnelDto forcePersonnelDto = forcePersonnelService.getForcePersonnelDetailsByForcePersonnelId(candidateForcePersonnelId).get();
		...
		if (!errors.isEmpty()) {
			...
			if(isAppointmentValid==5){
				...
				if(appointmentAmeRepo.save(appointmentAme)!=null) {
						...
						smsTemplateDto.setYear(AMEYear);
						SmsResponse smsresponse = smsTemplateService.rescheduleAppointmentSMS(smsTemplateDto,
								SMSConfigurationConstantsAME.DLT_TEMPLATE_ID_RESCHEDULING_APPOINTMENT);
						 if(smsresponse!=null) {
